...
<h1 class="h3 mb-2 text-gray-800">Edit Room Details</h1>
    <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                ...
                                name="room_number" value="{{ $room->room_number }}" placeholder="Room Number">
                                @error('room_number')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                <select name="floor_id" class="form-control  @error('floor_id') is-invalid @enderror" id="">
                                    <option value=""></option>
                                    @foreach ($floor as $item)
                                    <option value="{{ $item->id }}" @if($item->id == $room->floor_id) @selected(true) @endif>{{ $item->number }}</option>
                                    @endforeach

                                </select>

                                {{-- <input type="text" class="form-control  " id="floor"
                                    name="floor" value="{{ $room->floor }}" placeholder="Floor Number"> --}}
                                @error('floor')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                               <select style="width: 100%;" name="status" id="status" class="select2 @error('status') is-invalid @enderror">
                                <option value=""></option>
                                <option value="1" @if ($room->status==1)) @selected(true)  @endif>Available (Inventory Complete)</option>
                                <option value="2" @if ($room->status==2)) @selected(true)  @endif>Available (Inventory Incomplete)</option>
                                <option value="20" @if ($room->status==20)) @selected(true)  @endif>Occupied</option>
                                <option value="30" @if ($room->status==30)) @selected(true)  @endif>Under Maintenance</option>
                               </select>
                                @error('status')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                ...
                                @foreach ($items as $item)
                                        <div class="col-md-4 form-check">
                                            <label class="label label-defalt">
                                                <input class="form-check-input item-checkbox" type="checkbox" name="items[]" @if(in_array($item->id,$items_selected)) @checked(true)  @endif
