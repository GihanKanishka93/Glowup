...
class admissionCheckOutDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('id', function ($item) {
                return '<a href="' . route('admission.show', ['id' => $item->id, 'type' => 'past']) . '">' . $item->id . '</a>';
            ...
            ->addColumn('user_name', function ($item) {
                ...
                return '<span class="badge badge-info user-popup" style="cursor: pointer;" data-user-info="' . htmlspecialchars(json_encode($userInfo)) . '" data-updated-at="' . $item->updated_at . '" log-type="' . $logType . '">' . $userName . '</span>';

            })
            ->addColumn('action', function ($item) {
                $user = Auth()->user();
                $btn = '';
                // undo discharge action
                if ($user->can('undo-discharge')) {

                    $btn .= '<button title="Undo Discharge" data-bs-toggle="tooltip"  class="btn btn-circle btn-sm undo-discharge-btn" data-id="' . $item->id . '"><i class="fa fa-undo fa-2xl"></i>  </button>&nbsp;';

                }
                if ($user->can('daily-visit-list')) {
                    $btn .= '<a class="btn btn-primsary btn-circle btn-sm" data-bs-toggle="tooltip" href="' . route('daily-visit.create', $item->id) . '?type=checkout" title="Daily Notes"><i style="color:#183153" class="fa-solid fa-notes-medical fa-2xl"></i></a> ';
                }
               

                // discharge action
                if ($user->can('discharge')) {
                    $btn .= '<button class="btn btn-circle btn-sm discharge-btn" data-id="' . $item->id . '" data-bs-toggle="tooltip" title="Discharge Information"><i  class="fa-solid fa-message-smile fa-2xl" style="color: #B197FC;"></i>  </button>&nbsp;';
    ...
    public function query(admission $model): QueryBuilder
    {
        ...
        return $model->select('admissions.*')
            ->where(function ($query) {
                $query->whereNotNull('date_of_check_out');
            })
            ->with([
                'patient',
                'createdBy',
            ...
            ->when($searchValue, function ($query) use ($searchValue) {
                $query->where(function ($query) use ($searchValue) {
                    ...
                    ->orWhere(function ($query) use ($searchValue) {
                        $query->whereNotNull('date_of_check_out')
                            ->whereNotNull('date_of_check_out')
    ...
    {
        return $this->builder()
            ->setTableId('admissioncheckout-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('patient_name')
                ...
                ->orderable(true)
                ->searchable(true),
            Column::make('room.room_number')->title('Room Number'),
    ...
    {
        return 'admissionCheckOut_' . date('YmdHis');
    }
}
