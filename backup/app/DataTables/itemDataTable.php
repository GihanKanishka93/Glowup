    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function($item){
                ...
                $btn .= '<a class="btn btn-circle btn-sm btn-info"  data-bs-toggle="tooltip" title="Edit" href="'.route('item.edit',$item->id).'"><i class="fa fa-pen"></i></a> ';
         ...
         if($user->can('item-delete')){ 
            $btn .=  '<button class="btn btn-danger btn-circle btn-sm delete-btn"  data-bs-toggle="tooltip" title="Delete" data-id="'.$item->id.'"> <i class="fa fa-trash-alt"></i></button>';
                $btn .= '<form  action="'. route('item.destroy',$item->id).'" method="POST" class="d-inline"  id="del'.$item->id.'" >
