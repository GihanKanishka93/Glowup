                                    ...
                                    <li class="cashier-queue-item {{ $bill->is_new ? 'is-new' : '' }}">
                                        <div class="queue-meta">
                                            ...
                                            <div class="queue-subtitle">
                                                {{ $bill->treatment->pet->name ?? 'Unknown' }} · {{ $bill->treatment->doctor->name ?? 'Doctor N/A' }}
                                                @if($bill->is_new)
                                                    <span class="badge queue-badge-new">New</span>
                                                @endif
                                            </div>
                                            <div class="queue-foot">{{ \Illuminate\Support\Carbon::parse($bill->billing_date)->format('d M Y') }} · {{ number_format($bill->total ?? 0, 2) }}</div>
                                        </div>
                                        <div class="queue-actions btn-group">
                                            <a href="{{ route('billing.show', $bill->id) }}" class="btn btn-outline-primary btn-sm" title="Open bill">
                                                <i class="fas fa-folder-open"></i>
                                            </a>
                                            <a href="{{ route('billing.print', ['id' => $bill->id]) }}" target="_blank" class="btn btn-primary btn-sm">
                                                <i class="fas fa-print me-1"></i>Print
                                            </a>
                                            <form action="{{ route('billing.email', $bill->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                <button type="submit" class="btn btn-warning btn-sm text-dark" title="Email bill to owner">
                    ...
                    <div class="card shadow-sm border-0 h-100 cashier-history-card">
                        <div class="card-header border-0 d-flex flex-wrap align-items-center justify-content-between gap-2">
                            ...
                            <div class="btn-group cashier-filter" role="group" aria-label="Filter bills">
                                <button type="button" class="btn btn-outline-primary btn-sm active" data-filter="ready">Ready to Print</button>
                                <button type="button" class="btn btn-outline-primary btn-sm" data-filter="new">New</button>
                                <button type="button" class="btn btn-outline-primary btn-sm" data-filter="printed">Printed</button>
                                <button type="button" class="btn btn-outline-primary btn-sm" data-filter="unpaid">Awaiting Payment</button>
...
@push('styles')
    <style>
        ...
        .cashier-dashboard .metric-card.metric-ready .metric-icon { background: rgba(37, 99, 235, 0.12); color: var(--primary); }
        .cashier-dashboard .metric-card.metric-new .metric-icon { background: rgba(249, 115, 22, 0.12); color: #f97316; }
        .cashier-dashboard .metric-card.metric-printed .metric-icon { background: rgba(16, 185, 129, 0.12); color: #0f9f6d; }
        .cashier-dashboard .metric-card.metric-outstanding .metric-icon { background: rgba(78, 205, 196, 0.12); color: #0f766e; }
        .cashier-dashboard .metric-card.metric-cash .metric-icon { background: rgba(45, 212, 191, 0.12); color: #0d9488; }
        .queue-card {
            background: var(--surface);
            border: 1px solid rgba(148, 163, 184, 0.25);
            border-radius: 1rem;
