                        ...
                        <div id="vaccination" class="form-group row">
                            ...
                            @foreach ($vaccinationInfo as $vaccination)
                                <div class="previous-vaccination-details form-group row" id="vaccination-{{ $vaccination->id }}">
                                    <div class="col-md-4">
                                        <select name="vaccine_name[]" id="vaccine_name" class="select2 form-control vaccine_item @error('vaccine_name') is-invalid @enderror">
                                            ...
                                            @foreach ($vaccine as $item)
                                                <option value="{{ $item->id }}" @if($vaccination->vaccine_id == $item->id) selected @endif>
                                                    {{ $item->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-1">
                                        <button type="button" class="btn btn-danger btn-sm mt-0 mb-2 removePerson" data-id="{{ $vaccination->id }}">
                                            <i class="fa fa-trash"></i>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-md-4">
                                        <select name="vaccine_name[]" id="vaccine_name" class="select2 form-control vaccine_item @error('vaccine_name') is-invalid @enderror">
                                            <option value="" selected="selected"></option>
                                            @foreach ($vaccine as $item)
                                                <option value="{{ $item->id }}" @if(old('vaccine_name') == $item->name) selected @endif>
                                                    {{ $item->name }}
                                                </option>
                                            @endforeach
                                        </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add click event listener to remove buttons for existing prescriptions
    document.querySelectorAll('.removePerson').forEach(function(button) {
        button.addEventListener('click', function() {
            let prescriptionId = this.getAttribute('data-id');
            // Add your AJAX request here to delete the prescription from the server
            // $.ajax({
            //     url: `/prescription/${prescriptionId}`,
            //     type: 'DELETE',
            //     success: function(result) {
            //         // Remove the prescription element from the DOM
            //         document.getElementById(`prescription-${prescriptionId}`).remove();
            //     }
            // });
            document.getElementById(`prescription-${prescriptionId}`).remove(); // Remove from DOM for now
        });
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.removePerson').forEach(function(button) {
        button.addEventListener('click', function() {
            let vaccinationId = this.getAttribute('data-id');
            // Add your AJAX request here to delete the vaccination from the server
            // $.ajax({
            //     url: `/vaccination/${vaccinationId}`,
            //     type: 'DELETE',
            //     success: function(result) {
            //         // Remove the vaccination element from the DOM
            //         document.getElementById(`vaccination-${vaccinationId}`).remove();
            //     }
            // });
            document.getElementById(`vaccination-${vaccinationId}`).remove(); // Remove from DOM for now
        });
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('addService').addEventListener('click', function() {
        ...
        if (plusButton) {
            let removeButton = document.createElement('button');
            removeButton.type = 'button';
            removeButton.className = 'btn btn-danger btn-sm mt-0 mb-2 removeBillItem';
            removeButton.innerHTML = '<i class="fa fa-trash"></i>';
            plusButton.replaceWith(removeButton);
    ...
    // Attach click event to existing remove buttons to handle deletion
    document.querySelectorAll('.removeBillItem').forEach(function(button) {
        button.addEventListener('click', function() {
