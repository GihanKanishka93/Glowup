...
@section('content')
    ...
    @endphp
    <x-list-page title="Billing Report" :back-route="url()->previous()" :subtitle="'From ' . $startDate . ' to ' . $endDate" class="reports-monthly">
        <div class="report-shell">
            <div class="report-filters">
                <form id="dateRangeForm" action="{{ route('report.monthly-report') }}" method="get" class="row g-3">
                    @csrf
                    @method('GET')
                    <div class="col-12">
                    ...
                    <div class="col-sm-6 col-lg-3">
                        ...
                        <input type="text" id="start_date" name="start_date" class="datepicker form-control"
                            value="{{ old('start_date', $startDate) }}" placeholder="YYYY-MM-DD">
                    </div>
                    <div class="col-sm-6 col-lg-3">
                        <label for="end_date" class="form-label">End Date</label>
                        <input type="text" id="end_date" name="end_date" class="datepicker form-control"
                            value="{{ old('end_date', $endDate) }}" placeholder="YYYY-MM-DD">
                    </div>
                    <div class="col-sm-6 col-lg-3">
                        ...
                        <select id="doctor_id" name="doctor_id" class="form-select select2" data-placeholder="Any Doctor">
                            ...
                            @foreach ($doctors as $doctor)
                                <option value="{{ $doctor->id }}" @selected(old('doctor_id', $selectedDoctorId ?? null) == $doctor->id)>
                                    {{ $doctor->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col">
                    <div class="report-metric-card">
                        ...
                        <div class="metric-copy">
                            ...
                            <span class="metric-value" id="billCount">{{ number_format($billCount) }}</span>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <div class="report-metric-card">
                        ...
                        <div class="metric-copy">
                            ...
                            <span class="metric-value" id="billSum">{{ number_format($billSum, 2, '.', ',') }}</span>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <div class="report-metric-card">
                        ...
                        <div class="metric-copy">
                            ...
                            <span class="metric-value" id="billAverage">{{ number_format($averageBill, 2, '.', ',') }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
@section('third_party_scripts')
    ...
    <script>
        const reportChartInitialData = @json($reportCharts);
