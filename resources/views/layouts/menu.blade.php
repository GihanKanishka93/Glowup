@php
    ...
    $sidebarMetrics = Cache::remember('sidebar.metrics.' . ($user?->id ?? 'guest'), now()->addMinutes(5), function () {
        ...
        return [
            ...
            'petCount' => Pet::query()->whereNull('pets.deleted_at')->count(),
    ...
    $quickActions = array_values(array_filter($quickActions, fn ($action) => $abilityAllows($action)));
    ...
    $navSections = [
        ...
        [
            ...
            'items' => [
                [
                    'label' => 'Client List',
                    'icon' => 'fas fa-user',
                    'route' => route('pet.index'),
                    'active' => request()->routeIs('pet.index'),
                    'badge' => $sidebarMetrics['petCount'] ?? 0,
                ...
                [
                    ...
                    'icon' => 'fas fa-user-plus',
                    'route' => route('pet.create'),
                    'active' => request()->routeIs('pet.create'),
                    'can' => 'pet-create',
                ],
            ],
        ],
        ...
        [
            ...
            'items' => [
                ...
                [
                    'label' => 'Treatment Plan Sales',
                    'icon' => 'fas fa-syringe',
                    'route' => route('report.vaccination-sales'),
                    'active' => request()->routeIs('report.vaccination-sales'),
                ],
            ],
        ],
        [
            ...
            'items' => [
                ...
                [
                    ...
                    'can' => 'suspend-user-list',
                ],
                [
                    'label' => 'Skin Types',
                    'icon' => 'fas fa-droplet',
                    'route' => route('pet-category.index'),
                    'active' => request()->routeIs('pet-category.*'),
                    'can' => 'pet-category',
                ],
                [
                    'label' => 'Skin Concerns',
                    'icon' => 'fas fa-band-aid',
                    'route' => route('pet-breed.index'),
                    'active' => request()->routeIs('pet-breed.*'),
                    'can' => 'pet-breed',
                ],
                [
                    'label' => 'Services',
                ...
                [
                    'label' => 'Vaccinations',
                    'icon' => 'fas fa-syringe',
                    'route' => route('vaccination.index'),
                    'active' => request()->routeIs('vaccination.*'),
                    'can' => 'vaccination-list',
                ],
                [
                    'label' => 'Drug Catalog',
                    'icon' => 'fas fa-pills',
    ...
    <li class="sidebar-modern">
        <div class="sidebar-top">
            <div class="sidebar-brand-wrapper">
                <a class="sidebar-brand" href="{{ route('home') }}" aria-label="Navigate to home">
                    <img src="{{ asset('img/Glowup_Logo-modified.png') }}" alt="Glowup Skin Clinic" class="sidebar-brand-logo">
                </a>
                @if (!$isCashier)
                    <button class="sidebar-icon-button" type="button" data-command-open title="Search (⌘K)"><i class="fas fa-magnifying-glass"></i></button>
                @endif
            </div>
            ...
            @if (!empty($quickActions))
                <div class="sidebar-quick-actions">
                    @foreach ($quickActions as $action)
                        ...
                        @endphp
                        <a href="{{ $action['route'] }}" class="quick-action" data-quick-action data-label="{{ $action['label'] }}" data-href="{{ $action['route'] }}" data-icon="{{ $action['icon'] }}" title="{{ $action['label'] }}">
                    ...
                    @foreach ($section['items'] as $item)
                        @if (isset($item['children']))
                            ...
                            <li class="sidebar-node">
                                <button class="sidebar-link has-children {{ $isOpen ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $collapseId }}" aria-expanded="{{ $isOpen ? 'true' : 'false' }}" aria-controls="{{ $collapseId }}">
                                    <span class="sidebar-icon"><i class="{{ $item['icon'] ?? 'fas fa-circle' }}"></i></span>
                                    <span class="sidebar-label">{{ $item['label'] }}</span>
                                    @if ($hasBadge)
                                        <span class="sidebar-badge badge-{{ $item['badge_context'] ?? 'neutral' }}">{{ number_format($item['badge']) }}</span>
                                    @endif
                                    <span class="sidebar-caret"><i class="fas fa-chevron-right"></i></span>
                                </button>
                                <div id="{{ $collapseId }}" class="collapse sidebar-submenu {{ $isOpen ? 'show' : '' }}" data-bs-parent="#accordionSidebar">
                                    <ul>
                                        @foreach ($item['children'] as $child)
                                            ...
                                            @endphp
                                            <li>
                                                <a href="{{ $child['route'] }}" class="sidebar-sublink {{ !empty($child['active']) ? 'active' : '' }}" data-nav-item data-label="{{ $child['label'] }}" data-href="{{ $child['route'] }}" data-icon="{{ $item['icon'] ?? 'fas fa-circle' }}">
                        ...
                        @else
                            ...
                            @endphp
                            <li>
                                <a href="{{ $item['route'] }}" class="sidebar-link {{ !empty($item['active']) ? 'active' : '' }}" data-nav-item data-label="{{ $item['label'] }}" data-href="{{ $item['route'] }}" data-icon="{{ $item['icon'] ?? 'fas fa-circle' }}">
                                    <span class="sidebar-icon"><i class="{{ $item['icon'] ?? 'fas fa-circle' }}"></i></span>
                                    <span class="sidebar-label">{{ $item['label'] }}</span>
                                    @if (!empty($item['badge']))
                                        <span class="sidebar-badge badge-{{ $item['badge_context'] ?? 'neutral' }}">{{ number_format($item['badge']) }}</span>
        ...
        <div class="sidebar-footer">
            ...
            <div class="sidebar-account-card">
                <button class="sidebar-account-toggle dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false" data-current-status="{{ $currentStatusKey }}">
                    ...
                    <div class="sidebar-account-copy">
                        ...
                        <span class="sidebar-account-status status-{{ $currentStatusKey }}"><i class="fas fa-circle"></i><span class="user-status-label">{{ $currentStatusLabel }}</span></span>
                    </div>
                ...
                <ul class="dropdown-menu sidebar-account-menu">
                    ...
                    @foreach ($statusOptions as $value => $label)
                        <li>
                            <button type="button" class="dropdown-item d-flex align-items-center user-status-option {{ $value === $currentStatusKey ? 'active' : '' }}" data-status="{{ $value }}" data-label="{{ $label }}">
                                <span class="status-indicator {{ $value }}"></span>{{ $label }}
                            </button>
                        </li>
                    @endforeach
                    <li><hr class="dropdown-divider"></li>
                    <li><a class="dropdown-item" href="{{ route('users.profile') }}"><i class="fas fa-user"></i><span>Profile</span></a></li>
                    <li><a href="#" class="dropdown-item text-danger sidebar-logout-btn"><i class="fas fa-arrow-right-from-bracket"></i><span>Logout</span></a></li>
                </ul>
            </div>

            <div class="sidebar-bottom-actions">
                <button class="sidebar-icon-button" type="button" data-settings-open title="Open settings"><i class="fas fa-sliders-h"></i></button>
                <button class="sidebar-icon-button" id="sidebarToggle" type="button" aria-label="Collapse sidebar"><i class="fas fa-chevron-left"></i></button>
            </div>
        </div>
    </li>
</ul>
...
<div class="sidebar-settings-panel" id="sidebarSettingsFlyout" aria-hidden="true">
    <div class="settings-panel-header">
        ...
        <button type="button" class="sidebar-icon-button" data-settings-close aria-label="Close settings"><i class="fas fa-times"></i></button>
    </div>
...
<div class="command-palette" id="sidebarCommandPalette" role="dialog" aria-modal="true" aria-hidden="true">
    ...
    <div class="command-panel">
        <div class="command-header">
            ...
            <button type="button" class="sidebar-icon-button" data-command-close aria-label="Close command palette"><i class="fas fa-times"></i></button>
        </div>
...
<form id="sidebar-logout-form" action="{{ route('logout') }}" method="POST" class="d-none">
    ...
    <style>
        ...
        .badge-warning { background: rgba(249, 115, 22, 0.14); color: var(--warning); }
        .badge-info { background: rgba(56, 189, 248, 0.16); color: var(--info); }
        .badge-neutral { background: rgba(148, 163, 184, 0.2); color: var(--text-primary); }
        .sidebar-pill {
        ...
        .sidebar-account-status.status-available { color: #34d399; }
        .sidebar-account-status.status-in_consult { color: #fbbf24; }
        .sidebar-account-status.status-offline { color: #f87171; }
        ...
        body.theme-dark #accordionSidebar {
            background: linear-gradient(195deg, #050b1a 0%, #0f1a33 55%, #1b1b4b 100%);
        }
