...
class drugDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'drug.action')
            ->addColumn('action', function ($drug) {
                $user = Auth()->user();
                $btn = '';
                if ($user->can('drug-edit')) {
                    $btn .= '<a class="btn btn-circle btn-sm btn-info"  data-bs-toggle="tooltip" title="Edit" href="' . route('drug.edit', $drug->id) . '"><i class="fa fa-pen"></i></a> ';
                ...
                if ($user->can('drug-delete')) {
                    $btn .= '<button class="btn btn-danger btn-circle btn-sm delete-btn"  data-bs-toggle="tooltip" title="Delete" data-id="' . $drug->id . '"> <i class="fa fa-trash-alt"></i></button>';
                    $btn .= '<form  action="' . route('drug.destroy', $drug->id) . '" method="POST" class="d-inline"  id="del' . $drug->id . '" >
